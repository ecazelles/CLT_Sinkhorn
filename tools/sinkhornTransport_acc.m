function [D,L,u,v,alpha,beta]=sinkhornTransport_acc(a,b,C,U,lambda,stoppingCriterion,p_norm,tolerance,maxIter,VERBOSE)
...
%
% K is a d1 x d2 matrix, equal to exp(- M/lambda), where M is the d1 x d2
...
v=ones(size(a,1),size(b,2))/size(b,1);
...
%         end
        ...
        u_tilde = a ./ (K * v);      %prox
        u = u.^(1-theta).*(u_tilde).^(theta);   
     %   u_tilde=u_tilde.^2 ./ u;        %rprox
     %   u = u.^(1-alpha_u).*(u_tilde).^(alpha_u);   
        v_tilde = b ./ (K' * (u));   
        v = v.^(1-theta).*(v_tilde).^(theta);
       % v_tilde=v_tilde.^2./v;          %rprox(rprox)
       % v = v.^(1-alpha_v).*(v_tilde).^(alpha_v);
